...
\texttt{examdesign}\footnote{Dsiponible en CTAN,
...
\item No se pueden emplear preguntas relacionadas en bloques,
  compartiendo un enunciado común. En Moodle cada pregunta es
  independiente.  Puede recurrirse a copiar el enunciado común en
  todas ellas, pero no se puede garantizar que se presenten al
  estudiante en orden.
...
\item No podremos emplear la ventaja de incluir figuras generadas ``al
  ...
  \texttt{.Rnw}, ni tampoco código. En particular, el símbolo
...
\item Generar, siguiendo el procedimiento descrito en la
  ...
  Este examen no debe contener bloques de cuestiones o figuras.
...
\end{verbatim}
  ...
  comienzo y el final. Causaría un error algo como:
\begin{verbatim}
     \choice{
        Texto respuesta
     } \choice{
...
en que trabajamos, algo habitual en maquinas Linux; además, ha de ser
la versión 2. \texttt{de-macro} procede de
\url{https://ctan.org/pkg/de-macro}}.
 El output de
  \texttt{de-macro} es un fichero de nombre \texttt{EN.examen-clean.tex}.
...
\section{Licencias}
Todo el software mencionado es propiedad de sus
autores, licenciados en diferentes términos.
\texttt{moodleiza} y \texttt{exam2moodle.pl} son de fuente
libre bajo licencia GPL, y pueden obtenerse del repositorio GitHub
